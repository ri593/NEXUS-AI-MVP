...
def _load_system_prompt() -> str:
    try:
        ...
        return (
            "You are NEXUS AI, an advanced personal assistant. "
...
class NexusAILive:
    ...
    async def _listen_audio(self):
        ...
        def callback(indata, frames, time_info, status):
            with self._speaking_lock:
                nexus_ai_speaking = self._is_speaking
            if not nexus_ai_speaking and not self.ui.muted:
                data = indata.tobytes()
...
def main():
    ...
    def runner():
        ui.wait_for_api_key()
        nexus_ai = NexusAILive(ui)
        try:
            asyncio.run(nexus_ai.run())
